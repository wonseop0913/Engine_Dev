...
DescriptorHeapAllocator EditorGUIManager::_srvHeapDescAllocator;
...
EditorGUIManager* EditorGUIManager::GetInstance()
{
	if (s_instance == nullptr)
		s_instance = new EditorGUIManager();
	return s_instance;
}

Bulb::ProcessResult EditorGUIManager::Delete()
{
	if (s_instance != nullptr) {
		delete s_instance;
		s_instance = nullptr;
...
void EditorGUIManager::Render(ID3D12GraphicsCommandList* cmdList)
{
...
void EditorGUIManager::ShowEngineStatus()
{
...
void EditorGUIManager::ShowDebugSettings()
{
	ImGuiWindowFlags windowFlags =
		ImGuiWindowFlags_NoMove |
...
void EditorGUIManager::ShowHierarchyView()
{
	ImGuiWindowFlags windowFlags =
		ImGuiWindowFlags_NoMove |
		ImGuiWindowFlags_NoResize |
...
void EditorGUIManager::ShowInspectorView()
{
	ImGuiWindowFlags windowFlags =
		ImGuiWindowFlags_NoMove |
		ImGuiWindowFlags_NoResize |
	...
	if (ImGui::Begin("Inspector View", nullptr, windowFlags)) {
		...
		if (_selectedObj == nullptr) {
			...
			if (ImGui::Button("Set Parent")) {
				...
				_childQueueObj = _isParentSelectMode ? _selectedObj : nullptr;
			}
			ImGui::Separator();
...
void EditorGUIManager::ShowResourceDirectory()
{
	ImGuiWindowFlags windowFlags =
		ImGuiWindowFlags_NoMove |
		ImGuiWindowFlags_NoResize |
...
void EditorGUIManager::HierarchyObjectRecursion(shared_ptr<Transform> transform)
{
	...
	if (ImGui::IsItemClicked(0)) {
		if (_isParentSelectMode && _childQueueObj != nullptr) {
			_childQueueObj->GetTransform()->SetParent(transform);
...
void EditorGUIManager::ShowMainMenuBar()
{
...
void EditorGUIManager::ShowMenuFile()
{
...
void EditorGUIManager::ShowMenuEdit()
{
...
void EditorGUIManager::ShowPreferences()
{
...
void EditorGUIManager::ShowView()
{
...
void EditorGUIManager::DrawGizmo()
{
	if (_selectedObj != nullptr) {
...
void EditorGUIManager::ToggleWindows()
{
